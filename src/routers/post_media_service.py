...
from util import parsing_xml, create_content_xml
...
async def post_media_service(request: Request):
    ...
    logging.info(Fore.BLUE + ' main | post_media_service | soapaction from headers: %s \n '  % (request.headers['soapaction']) + Style.RESET_ALL)  
    logging.info(Fore.BLUE + ' main | post_media_service | content: %s \n '  % (etree.tostring(content)) + Style.RESET_ALL)
    ...
    match request.headers['soapaction']:
        case '"http://www.onvif.org/ver10/media/wsdl/GetProfiles"':
            ...
            headers = {'Server': 'gSOAP/2.8E',
                       ...
                       'Date': 'Thu, 07 Mar 2024 02:07:44 GMT'}
            response = Response(media_type = media_type, 
                            status_code = status_code,
                            content = content,
                            headers=headers)
            logging.info(Fore.BLUE + ' main | post_media_service | response headers: %s \n '  % (response.headers) + Style.RESET_ALL)
            return response